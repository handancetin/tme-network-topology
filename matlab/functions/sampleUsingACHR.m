function sampleUsingACHR(cell_models, nStepsPerPoint, nPointsReturned)
    ...
    for i = 1:length(cell_models)  
        ...
        [~, curSamples] = sampleCbModel(curModel, [], 'ACHR', options, curModel);
        
        % Save at each iteration 
        writematrix(curSamples, fullfile('../output/samples/', sprintf('ACHR_%s.csv', cell_name))); 
    end
end
